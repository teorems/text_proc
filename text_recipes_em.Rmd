...
model_rr_liked <- textTrain(rr_emb$texts$Review, 
                              rr$Liked)
...
<!-- ## Tracer les mots statistiquement significatifs  -->
<!-- Le paquet texte propose plusieurs façons de tracer les mots ; nous utiliserons ici le tracé dimensionnel supervisé. Le tracé se fait en deux étapes : Tout d'abord, la fonction `textProjection` pré-traite les données, y compris le calcul des statistiques pour chaque type de mot à tracer. Ensuite, la fonction `textProjectionPlot()` visualise les mots, en incluant de nombreuses options pour définir la couleur, la police, etc. de la figure. Diviser cette procédure en deux étapes rend le processus plus transparent (puisque l'utilisateur peut naturellement voir la sortie selon laquelle les mots sont tracés) et plus rapide puisque les calculs les plus lourds sont effectués dans la première étape, la dernière étape est plus rapide de sorte que l'on peut essayer différents paramètres de conception.  -->
...
<!-- ### textProjectionPlot(): A nuage de mot en deux dimensions -->
...
<!--   x_axes_label = "Not Liked vs. Liked", -->
